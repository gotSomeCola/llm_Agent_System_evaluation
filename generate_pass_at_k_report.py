...
def generate_summary_stats(results, output_file, model_name, k_override=None):
    ...
    Args:
        ...
        k_override: Override k value for recalculation (e.g., 3 instead of using n_samples)
    """
    total = len(results)
    if total == 0:
    ...
    # Extract n_samples from data
    n_samples = results[0].get('n_samples', 1) if results else 1
    
    # Determine k for reporting
    if k_override is not None:
        k = k_override
        print(f"Recalculating Pass@K metrics with k={k} (original n_samples={n_samples})")
    else:
        k = n_samples
    ...
    if k_override is not None:
        # Recalculate pass@1, pass@k with new k value
        pass_1_values = []
        pass_k_values = []
        for r in results:
            n = r.get('n_samples', 1)
        ...
        avg_pass_1_recalc = sum(pass_1_values) / total
        avg_pass_k = sum(pass_k_values) / total
        
        # Pass@5 only if k >= 5
        if k >= 5 or n_samples >= 5:
            pass_5_values = []
            for r in results:
                n = r.get('n_samples', 1)
                c = r.get('c_correct', 0)
        ...
        # Use existing values from results
        avg_pass_5 = sum(r.get('pass@5', 0) for r in results) / total if n_samples >= 5 else None
    ...
    if successful_tasks:
        ...
        avg_code_bleu = avg_code_bert = 0.0
...
def save_summary_csv(stats, output_file=None):
    ...
    with open(output_file, 'a', newline='', encoding='utf-8') as f:
        fieldnames = [
            ...
            'pass@5_estimated_%', 'pass@k_estimated_%', 'any_pass_count',
...
def main():
    ...
    parser.add_argument('--k', type=int, default=None, help='Override k value for recalculation (default: use n_samples from data)')
    ...
    stats = generate_summary_stats(results, summary_file, model_name, k_override=args.k)
